...
def visual(background, img_path, interval, iterations):
    ...
    display_image(rgb,"Inundation simulation with rainfall intensity 5 cm/h", max_plot=False)
    plt.pause(3)
    layer = None
    for iter in range(1, iterations+1):
        ...
        for i, img in enumerate(img_files):
            ...
            if iter % 2 != 0:
                text = plt.text(cols/2,-5, "Water level increasing: Time = {} hours".format(i+1), fontsize=20, ha = 'center')
            else:
                text = plt.text(cols/2,-5, "Water level decreasing ...", fontsize=20, ha = 'center')
            plt.pause(interval)
            text.remove()
    return True
