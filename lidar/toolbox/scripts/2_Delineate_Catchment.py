...
def delineate_catchment(in_dem, in_sink, out_catchment):
    ...
    arcpy.PolygonToRaster_conversion(in_sink, FieldOID, SinkRaster, "CELL_CENTER", "NONE", cell_size)
    ...
    arcpy.Dissolve_management(Catchment_select,out_catchment,dissolve_field="GRIDCODE", statistics_fields="", multi_part="MULTI_PART", unsplit_lines="DISSOLVE_LINES")

    area_field = "cat_area"
    arcpy.AddField_management(out_catchment, area_field, "DOUBLE")
    arcpy.CalculateField_management(out_catchment,area_field,"!shape.area@squaremeters!","PYTHON_9.3","#")

    arcpy.JoinField_management(in_sink,in_field="ID",join_table=Watershed, join_field="Value", fields="Count")
    arcpy.AddField_management(in_sink,field_name="cat_area",field_type="FLOAT")
    arcpy.CalculateField_management(in_sink, field="cat_area", expression="!Count_1! * math.pow(" + str(cell_size) + ",2)", expression_type="PYTHON_9.3")
    ...
    arcpy.CalculateField_management(in_sink, field="dep2catR", expression="!AREA! / !cat_area!", expression_type="PYTHON_9.3")
