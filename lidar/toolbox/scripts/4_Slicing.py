...
def levelSet(
    ...
    ):  # slicing operation using to top-down approach
        ...
        for i, object in enumerate(objects):
            ...
            if len(parent_ids) == 0:  # This is the first depression, maximum depression
                ...
                if catchment_img is not None:
                    catchment_area = catchment_img[
                        row, col
                    ]  # get the catchment size of the depression
...
if __name__ == "__main__":
    ...
    arcpy.AddMessage(
        "min_elev = {}, max_elev = {}, no_data = {}".format(min_elev, max_elev, no_data)
    )
    ...
    prep_time = time.time()
    arcpy.AddMessage(
        "Data preparation time: {:.4f} seconds".format(prep_time - init_time)
    )
    ...
    arcpy.AddMessage(
        "Identify level time:\t {:.4f}".format(time.time() - identify_time)
    )
