...
def DelineateCatchment(DEMRasterPath, flow_direction, SinkPolyPath, OutputPath):
    ...
    arcpy.PolygonToRaster_conversion(
        SinkPolyPath, FieldOID, SinkRaster, "CELL_CENTER", "NONE", cell_size
    )
...
def calculateArea(in_shp, fieldname):
    ...
    arcpy.CalculateField_management(
        in_shp, fieldname, "!shape.area@squaremeters!", "PYTHON_9.3"
    )
    ...
    for shp in shapefiles:
        if shp.endswith(".shp"):
            ...
            arcpy.AddField_management(
                shp_path, field_name="dep2catR", field_type="FLOAT"
            )
...
if __name__ == "__main__":
    ...
    arcpy.PolygonToRaster_conversion(
        out_merge_levels,
        value_field="cat_area",
        out_rasterdataset=tmp_catchment_img,
        cellsize=cell_size,
    )
